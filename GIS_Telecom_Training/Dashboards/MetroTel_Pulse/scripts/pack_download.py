#!/usr/bin/env python3
"""Build a download site + zip of MetroTel Pulse (no pycache)."""
...
SITE = Path("/tmp/metrotel-pulse-site")
ARTIFACTS = Path("/opt/cursor/artifacts")
ZIP_NAME = "MetroTel_Pulse_Dashboard_Template.zip"
...
<head>
  ...
  <title>Download MetroTel Pulse</title>
  <style>
    :root {{
...
<body>
<main>
  <h1>MetroTel Pulse — download pack</h1>
  <p>All dashboard files, Python scripts, sample data, and directions in one zip.</p>
  <div class="hero">
...
def main() -> None:
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for path in files:
            zf.write(path, arcname=f"MetroTel_Pulse/{path.relative_to(ROOT).as_posix()}")
    ...
    shutil.copy2(SITE / "index.html", ARTIFACTS / "metrotel_pulse_download.html")
    shutil.copy2(ROOT / "START_HERE.md", ARTIFACTS / "START_HERE.md")
