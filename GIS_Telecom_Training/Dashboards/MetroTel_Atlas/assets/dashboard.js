(() => {
  ...
  function renderKpis() {
    ...
    $("kpi-subs-hint").textContent = state.selectedZone === "ALL"
      ? "Network view · subscriber-weighted ARPU so CBD counts more than Tech Park."
      : `Filtered to ${state.selectedZone} · same weighting rules as the dark Pulse board.`;
    $("kpi-churn").style.color = k.churn >= 0.35 ? "var(--clay)" : "var(--ink)";
    $("cov-bar").style.width = `${Math.max(0, Math.min(100, k.cov))}%`;
    $("cov-bar").style.background = k.cov < 85 ? "var(--gold)" : "var(--sage)";
  }

  function ensureChart(id, spec) {
    if (state.charts[id]) state.charts[id].destroy();
    state.charts[id] = new Chart($(id), spec);
  }
    ...
    ensureChart("chart-arpu", {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: rows.map((r) => PRIORITY_COLOR[r.PRIORITY] || "#3d5a80"),
          borderRadius: 5,
        }],
      },
      options: {
    ...
    ensureChart("chart-trend", {
      ...
      options: {
        ...
        scales: {
          ...
          y1: { position: "right", ticks: { color: "#c45c26" }, grid: { display: false } },
        },
      },
    });
  }
  ...
  function renderList() {
    ...
    $("decision-list").innerHTML = rows
      .map((d) => `<article class="row" data-zone="${d.ZONE}">
        <div>
          <span class="pill ${String(d.PRIORITY).toLowerCase()}">${d.PRIORITY}</span>
  ...
  function initMap() {
    ...
    state.zoneLayer = L.geoJSON(JSON.parse(JSON.stringify(window.PULSE_DATA.zones)), {
      style: zoneStyle,
  ...
  function refreshMapLayers() {
    ...
    for (const site of window.PULSE_DATA.sites) {
      ...
      L.circleMarker([site.LAT, site.LON], {
        ...
        color: candidate ? "#c45c26" : "#1b2430",
        fillColor: candidate ? "#c45c26" : "#2f6f5e",
        fillOpacity: 0.95,
        weight: 1.4,
      })
    ...
    if ($("toggle-subs") && $("toggle-subs").checked) {
  ...
  function fillFilters() {
    $("filter-zone").innerHTML = `<option value="ALL">All zones</option>` +
      window.PULSE_DATA.kpis.map((r) => `<option value="${r.DIST_ID}">${r.DIST_NAME}</option>`).join("");
    const prios = [...new Set(window.PULSE_DATA.kpis.map((r) => r.PRIORITY))];
    $("filter-priority").innerHTML = `<option value="ALL">All priorities</option>` +
      prios.map((p) => `<option value="${p}">${p}</option>`).join("");
    const fibers = [...new Set(window.PULSE_DATA.kpis.map((r) => r.FIBER_STATUS || "").filter(Boolean))];
    $("filter-fiber").innerHTML = `<option value="ALL">All fiber</option>` +
  ...
  function wireEvents() {
    $("filter-zone").addEventListener("change", (e) => { state.selectedZone = e.target.value; refresh(); });
    $("filter-priority").addEventListener("change", (e) => { state.priority = e.target.value; refresh(); });
    $("filter-fiber").addEventListener("change", (e) => { state.fiber = e.target.value; refresh(); });
    ...
    $("file-kpi").addEventListener("change", (e) => {
      ...
      if (file) file.text().then(ingestZoneCsv);
    });
  ...
  function boot() {
    if (!window.PULSE_DATA) {
      document.body.innerHTML = "<p style='padding:24px'>Missing assets/embedded-data.js.</p>";
      return;
    }
    fillFilters();
    wireEvents();
  ...
  window.MetroTelAtlas = { refresh, ingestZoneCsv, computeKpis, filteredKpis };
